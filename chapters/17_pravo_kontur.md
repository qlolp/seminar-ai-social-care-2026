...
**Факт.** У ИИ-услуги нужен человек-владелец, не только дежурный по аптайму. Сервер может отвечать за доли секунды и при этом уверенно врать; сломанному боту хотя бы не верят. Без хозяина люди звонят перепроверить чат — самообслуживания нет<sup>129</sup>.

**Рекомендация.** Ответственный за цифровые проекты — и есть этот хозяин: база, куда ходит помощник, эскалация, разбор ошибок. Аптайм качеством не считается.
...
**Документы (А):** приказ с регламентом (приложение 3), список допущенных сервисов, журнал инцидентов. Если помощник не только пишет черновик, а ещё отправляет или правит — в том же приказе матрица: что нельзя, что человек подтверждает, что можно. Журнал этих решений потом и проверяют<sup>99</sup>. Если рядом с приказами появляется локальный агент — опросник, выжимка почты, расшифровка планёрки, поиск по своей базе, контролёр полноты, — это тот же закрытый контур: человек утверждает, смысл держит координатор, не «замена старшей смены»<sup>123</sup>. Обзор регулирования ИИ в России называет тот же пробел: локальные акты организации и статус «ИИ-сотрудника» закон пока не закрывает. ФСТЭК — по тексту, не по чужому конспекту<sup>135</sup>.
...
Обработка персональных данных требует уведомления уполномоченного органа. Для большинства учреждений соцобслуживания уведомление уже подано, но при появлении новых целей обработки (видеоаналитика, датчики состояния) его нужно актуализировать. Это недорогая и быстрая процедура, которую, однако, вспоминают обычно после начала пилота — и тогда она превращается в нарушение. Правило простое: изменился перечень систем или целей — проверь уведомление. Если персональные данные уходят в иностранный сервис, это уже второе уведомление — о трансграничной передаче, не то же самое, что «мы вообще обрабатываем данные» (раздел 10.3а)<sup>103</sup>. Биометрия (распознавание лиц) — отдельная история: как сказано в разделе 10, доклад рекомендует строить системы так, чтобы биометрии не было вовсе.
...
**Рекомендация.** Если вендор обещает агента к вашим журналам, три вопроса службы безопасности идут в тот же разговор: кто выдал право на операцию, как забрать, что останется в журнале через месяц<sup>127</sup>.

Пользовательское соглашение зарубежного чата — не договор поручения. Суд уже назвал выгрузку в нейросеть разглашением третьему лицу, не «работой в своём окне» (раздел 10.3а)<sup>103</sup>.
...
**Факт.** На 1 сентября 2026 года у ФСТЭК уже есть действующие методички и приказ № 117 (в редакции, вступившей в силу 01.09.2026), где искусственный интеллект назван отдельным предметом защиты; есть и проекты правок — в том числе про агентов. Обзор на Хабре — ориентировка, не юридическое заключение<sup>86</sup>. Цепочка редакций не кончается: № 17 сменил № 117, тот уже читают в редакции № 137. Индекс без даты редакции ответит по отменённому акту (раздел 9.7а)<sup>111</sup>.

**Факт.** Календарный срок «устранить за N дней» на ИИ-стеке ломается: «устранить» значит разное — патч, мажорный апгрейд, прошивка, «не будем чинить», веса без номера уязвимости. Зарубежный ориентир считает срок по риску актива; копировать его не нужно. В РФ методика ФСТЭК от 30.06.2025 и приказ № 117 (с 01.03.2026) уже называют критические — не дольше суток, высокие — не дольше семи дней; компенсирующие меры — наравне с патчем; сам апдейт тоже риск. Дерево решений — до срока; митигация — под подпись<sup>134</sup>.

**Интерпретация.** Это не новый закон «для домов ухода» и не обязанность каждого интерната строить контур государственной информационной системы. Но фраза вендора «ИИ пока не регулируется» для государственного учреждения уже неверна. Тот же тон — у судебной Концепции: ИИ помогает, человек решает; этапы без оценки не масштабируют (раздел 10.1)<sup>116</sup>. Для директора ДСО срок и компенсирующие меры — ориентир службе информационной безопасности и юристу, не копия зарубежного календаря и не заключение.

**Рекомендация.** Юрист сверяет, попадаете ли вы в контур приказа № 117 (госучреждение, ГИС) и что в договоре с вендором сказано про защиту при использовании ИИ. Формулировки в приказ — с официального текста, не с обзора. Рецепт закупки «безопасной разработки» отсюда не следует.
