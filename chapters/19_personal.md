...
На «заменят» — правдой: ИИ в наших задачах не заменяет уход, он убирает бумажную и сторожевую работу. Штат не сокращается, и это решение директора объявляется вслух до запуска. Тот же ответ — про «агента-стратега»: в закрытом контуре он собирает и сверяет, смысл держит человек. Это продолжение старшей смены, не её замена<sup>123</sup>. Агент от имени дома — цифровой сотрудник: права по задаче, не «ко всем журналам»; читать можно, менять и отправлять — отдельно, с подтверждением<sup>128</sup>. Агент замечает раньше смены — не решает за директора; критерий «готово» пишет человек до старта, не агент в отчёте (раздел 16.1)<sup>131</sup>.
...
**Факт.** Работа с ИИ добавляет слой, которого раньше не было: оператор держит не только результат, а ещё кому что отдано и куда вернуться. Факты проверяют по цене ошибки: цифра, дата, цитата, медицина и право — к первоисточнику; заголовок — нет. Критерии приёмки пишут заранее, не «сделай хорошо». Сэкономленные минуты легко съедает следующая задача; точку остановки задают до старта, не «когда получится гениально»<sup>114</sup>.

**Рекомендация.** Цифровая гигиена персонала — привычка открыть источник и знать, когда закрыть вкладку. Не канцелярит «будьте внимательны». Не вводите KPI «сколько раз открыли помощника» или «сколько знаков сожгли». Спрашивайте, что дошло до жителя и что смена приняла (раздел 16.1)<sup>124</sup>.
...
**Факт.** В открытом протоколе агента Битрикс24 опасное действие не исполняется по тексту чата: сначала план, затем человек подтверждает его отдельно и «на этот раз»; молчание и прошлое «да» согласием не считаются; права агента не шире прав текущего сотрудника<sup>89</sup>. Открытая методология говорит то же словами: «ИИ предлагает, человек решает, программа исполняет». Рекомендация модели не есть разрешение на действие; утверждение — на этот раз, с целью и сроком<sup>96</sup>.

**Рекомендация.** Тот же порядок — для любого помощника рядом с журналами и карточками: прочитать можно, изменить — только после того, как человек подтверждает конкретное действие, человек решает. Платформу и методологию не копируем. Границу «код исполняет / модель предлагает» записывают явно: иначе она уедет в промпт и начнёт плыть (раздел 21.6)<sup>108</sup>.

**Факт.** Фоновый помощник первой линии писал скрытый комментарий-подсказку в заявку: решение оставалось у оператора. Когда агент попробовал действовать сам — упёрся в карточку согласия без живого пользователя. Создать заявку под чьей-то учёткой — уже действие, не совет<sup>118</sup>.

**Рекомендация.** Подсказку в заявку или журнал дома можно оставить. Необратимое действие — только с правилом подтверждения, записанным заранее. Это не рекомендация Microsoft Copilot Studio.

**Факт.** «Агент тупит» чаще диагноз инструкции, а не модели. Три дефекта: правило написано прозой; написано как «не надо», без «делай вот так»; у правила нет проверки. Жёсткое правило живёт в исполняемом реестре и на страже записи, не только в тексте. Когда документы спорят, агент чинит удобный: иерархию называют явно — ТЗ нормативное, описание кода нет. Отчёт «все тесты зелёные» без охвата не принимают. Пока ослабление теста ничего не стоит, формулировка не работает: нужен гейт, который краснеет<sup>112</sup>.

**Рекомендация.** Для помощника дома то же: что нельзя — в приказе и в проверке, не в лозунге. Человек подтверждает действие. Платформу автора не копируем.

**Интерпретация.** Модель без обвязки — малая доля результата. Регламент, человек на приёмке, журнал — и есть экосистема. Без людей, которые умеют утверждать черновик, копилот в коридоре не работает<sup>125</sup>. У самой услуги тоже нужен живой хозяин: иначе аптайм зелёный, а ответам не верят (раздел 17.1)<sup>129</sup>.
